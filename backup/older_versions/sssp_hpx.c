...
_send_edge_handler(Vertex* v_, int edge, int weight)
{
    ...
    /*
      if(v->count >= EDGES_BUFF_SIZE){
        printf("Vertex ID: %d, EDGES_BUFF_SIZE exeeded, implement linklist! Not adding edge %d,
      count = %d\n",v->vertex_id, edge, v->count); hpx_lco_sema_v(v->vertex_local_mutex, HPX_NULL);
        unpin here
        return HPX_SUCCESS;
      }
...
_ack_handler(Vertex* v_, hpx_addr_t vertices, hpx_addr_t sssp_halting_LCO)
{
    ...
    if (v->deficit == 0 && v->parent == ROOT) {
        ...
        // v->distance_from_start); printf("vertex %d actions invocked is %d\n",v->vertex_id,
        // v->actions_invoked);
    }
...
static int
_reduce_actions_ack_handler(Vertex* v_,
                            hpx_addr_t vertices,
                            int partial_reduce,
                            hpx_addr_t reduce_halting_LCO)
{
    ...
    if (v->deficit == 0 && v->parent == ROOT) {
        ...
        v->actions_invoked, v->reduced_int_buffer);
        */
        v->reduced_int_buffer = 0;
        v->actions_invoked = 0;
    }
...
static int
_reduce_acks_handler(Vertex* v_,
                     hpx_addr_t vertices,
                     int sender_vertex,
                     hpx_addr_t reduce_halting_LCO)
{
...
void
REDUCE_actions_invoked_async(hpx_addr_t vertices, int start_vertex, hpx_addr_t reduce_done_lco)
{
