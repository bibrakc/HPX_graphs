...
typedef struct{
  ...
  unsigned long actions_invoked;
  unsigned long reduced_int_buffer;
...
static int _reduce_actions_ack_handler(Vertex *v_, hpx_addr_t vertices,
                        unsigned long partial_reduce, double partial_time, hpx_addr_t reduce_halting_LCO){
  ...
  if(v->deficit == 0 && v->parent == ROOT){
    ...
    printf("vertex %d reduced actions invocked is %lu, sum of action time is %lf\n",
...
static int _reduce_acks_handler(Vertex *v_, hpx_addr_t vertices,
  ...
  if(sender_vertex == ROOT && v->edges.head->count == 0){
    ...
    printf("vertex %d reduced actions invocked is %lu\n",v->vertex_id, v->reduced_int_buffer);
